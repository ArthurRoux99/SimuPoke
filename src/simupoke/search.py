...
from .sim import Mon, simulate_turn
...
def rank_actions(me: Mon, opp: Mon, field: FieldState | None = None, *,
    ...
    """Classe mes actions par valeur attendue à 1 tour (coups simultanés)."""
    ...
    for mv in my_cands:
        ...
        for omv in opp_cands:
            res = simulate_turn(me, opp, mv, omv, field, roll=roll, copy=True)
            score = evaluate_state(res.me, res.opp)
        ...
        actions.append(ActionValue(
            move=get_move(mv).name, expected=expected, worst=worst,
            ko_chance=ko_chance, survives_worst=survives_worst))
...
def _recommend(actions: list[ActionValue]) -> str:
    ...
    if safe and safe[0] is not best and safe[0].expected >= best.expected - 0.15:
        s = safe[0]
        return (f"{s.move} — meilleur compromis sûr "
    ...
    return f"{best.move} — meilleure valeur attendue{ko}{risk}."
